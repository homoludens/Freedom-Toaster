...
from PyQt4 import QtCore, QtGui, QtNetwork, QtXml, QtTest
#from progress import  Helper


class DisplayShape(QtCore.QObject):
    ...
    def animate(self):
        if not self.targetPos.isNull():
            ...
            newPosition = displacement.pointAt(0.2)
        ...
        if not "destroy" in self.metadata:
            if "fade" in self.metadata:
                ...
                if penAlpha != fadeMinimum or brushAlpha != fadeMinimum or self.metadata["fade"] > 0:
                    penAlpha = max(fadeMinimum, min(penAlpha + self.metadata["fade"], 50))
                    brushAlpha = max(fadeMinimum, min(brushAlpha + self.metadata["fade"], 50))
...
class TitleShape(DisplayShape):
    def __init__(self, text, font, pen, position, maxSize, alignment=QtCore.Qt.AlignVCenter|QtCore.Qt.AlignLeft):
        ...
        textWidth = max(fm.width(self.text), 200)
...
class ImageShape(DisplayShape):
    def __init__(self, original, position, maxSize, alpha=255, alignment=QtCore.Qt.AlignCenter):
        DisplayShape.__init__(self, position, maxSize)
...
class DocumentShape(DisplayShape):
    def __init__(self, text, font, position, maxSize, alpha= 120):
        DisplayShape.__init__(self, position, maxSize)
        ...
        self.source.fill(QtGui.qRgba(0, 0, 0, 50))
        ...
        painter.setRenderHint(QtGui.QPainter.TextAntialiasing)
        painter.setRenderHint(QtGui.QPainter.Antialiasing)
        painter.setBrush(QtGui.QBrush(QtGui.QColor(125, 125, 125, 120)))
        context.palette.setColor(QtGui.QPalette.Text, QtGui.QColor(255, 255, 255, 255))
...
#class ProgressShape(DisplayShape, QtGui.QWidget):
#    def __init__(self, helper, position, maxSize, alpha=255, alignment=QtCore.Qt.AlignCenter):
#        DisplayShape.__init__(self, position, maxSize)
...
#        self.goback=False
#        
#        timerP = QtCore.QTimer()
#        self.connect(timerP, QtCore.SIGNAL("timeout()"), self.animate)
#        timerP.start(50)
#        
#    def animate(self):
#        if not self.goback:
#            self.elapsed = (self.elapsed + 50) % 10000
#        else:
#            self.elapsed = (self.elapsed - 50) % 10000
#        
...
#            self.goback=False
#        print self.elapsed 
#        ev=QtCore.QRectF(100.0, 100.0, 50.0, 150.0)
#        self.redraw(ev)
#        
