...
class DisplayWidget(QtGui.QWidget):
    def __init__(self, parent=None):
        ...
        self.shape1=None
        self.pauseAnimation = QtCore.QTimer(self)
        self.pauseAnimation.setSingleShot(True)
        self.pauseAnimation.setInterval(1500)
        self.connect(self.pauseAnimation, QtCore.SIGNAL("timeout()"), self.appendShapeHelp)
        
    def appendShape(self, shape):
        if shape.animateWait == 0:
            self.shapes.append(shape)
        else:
            self.shape1=shape
#            print shape
#            self.connect(self.pauseAnimation, QtCore.SIGNAL("timeout()"), self.appendShapeHelp(shape))
            self.pauseAnimation.start()
            
            
        self.empty = False
        self.enableUpdates()
    
    def appendShapeHelp(self):
    ...
    def paintEvent(self, event):
        ...
        painter.begin(self)
        painter.fillRect(event.rect(),QtGui.QBrush(QtGui.QPixmap('images/demobg.png')))
        for shape in self.shapes:
    ...
    def dimmShapes(self, background, progress):
        self.insertShape(160, background)
        for shape in self.shapes:
            shape.metadata["fade"] = 15
            shape.metadata["fade minimum"] = 55
